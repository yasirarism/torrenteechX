    ...
    if os.path.isdir(local_file_name):
        ...
        if not message.photo:
            new_m_esg = await message.reply_text(
                f"Ditemukan {len(directory_contents)} file <a href='tg://user?id={from_user}'>🤒</a>",
                quote=True
        ...
        if os.path.getsize(local_file_name) > TG_MAX_FILE_SIZE:
            ...
            i_m_s_g = await message.reply_text(
                ...
                f"Ukuran file yang terdeteksi: {d_f_s} 😡\n"
                "\n🤖 Mencoba memecah file... 🌝🌝🌚"
            )
            ...
            await i_m_s_g.edit_text(
                f"Ukuran file terdeteksi: {d_f_s} 😡\n"
...
async def upload_to_gdrive(file_upload, message, messa_ge, g_id):
    ...
    del_it = await message.edit_text(
        f"<a href='tg://user?id={g_id}'>🔊</a> Sekarang mengupload ke ☁️ Cloud!!!"
    )
    ...
    if UPLOAD_AS_DOC.upper() == "TRUE":  # todo
        ...
        if not edit_media:
            message_for_progress_display = await message.reply_text(
                "Memulai upload {}".format(os.path.basename(local_file_name))
            )
        ...
        try:
            ...
            LOGGER.info(e)
            await message_for_progress_display.edit_text("**GAGAL**\n" + str(e))
        else:
            if message.message_id != message_for_progress_display.message_id:
                try:
                    if sent_message is not None:
